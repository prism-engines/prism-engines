    ...
    def analyze(
        ...
        **kwargs
        ...
        if metric == "correlation":
            ...
            # Handle NaN values (occur when an indicator has zero variance)
            # Replace NaN with 0 correlation (max distance of 1)
            corr_matrix = np.nan_to_num(corr_matrix, nan=0.0)
            
            dist_matrix = 1 - corr_matrix
            
            # Force symmetry to avoid floating-point precision errors
        ...
        for cluster_id, members in clusters.items():
            if len(members) > 1:
                ...
                cohesion = np.nanmean(corrs[mask])  # Use nanmean to handle NaN
