...
def test_runner_imports():
    ...
    try:
        ...
        print("[OK] CLIRunner imported")
        print("[OK] WorkflowExecutor imported")
        print("[OK] OutputManager imported")
        return True
...
def test_executor_initialization():
    ...
    try:
        ...
        print(f"[OK] Panel loader: {len(panels)} panels")

        # Test workflow loader
        workflows = executor.list_workflows()
        print(f"[OK] Workflow loader: {len(workflows)} workflows")

        # Test engine loader
        engines = executor.list_engines()
        print(f"[OK] Engine loader: {len(engines)} engines")
...
def test_output_manager():
    ...
    try:
        ...
        print(f"[OK] Created run directory: {run_dir}")
        ...
        print(f"[FAIL] Output manager test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_workflow_execution():
    ...
    try:
        ...
        if status == "completed":
            ...
            print(f"[FAIL] Workflow failed: {results.get('error')}")
            return False
...
def test_cli_list_commands():
    ...
    try:
        ...
        if result.returncode == 0 and "market" in result.stdout.lower():
            print("[OK] --list-panels works")
        else:
            print(f"[FAIL] --list-panels failed: {result.stderr}")
            return False
        ...
        if result.returncode == 0 and "regime" in result.stdout.lower():
            print("[OK] --list-workflows works")
        else:
            print(f"[FAIL] --list-workflows failed: {result.stderr}")
            return False
        ...
        if result.returncode == 0 and "pca" in result.stdout.lower():
            print("[OK] --list-engines works")
        else:
            print(f"[FAIL] --list-engines failed: {result.stderr}")
            return False

        return True

    except Exception as e:
        print(f"[FAIL] CLI list commands failed: {e}")
        return False
...
def test_cli_direct_mode():
    ...
    try:
        ...
        if result.returncode == 0:
            print("[OK] Direct mode execution completed")
            if "COMPLETED" in result.stdout or "completed" in result.stdout.lower():
                print("[OK] Execution status: completed")
            return True
        else:
            print(f"[FAIL] Direct mode failed with code {result.returncode}")
            print(f"   stderr: {result.stderr[:200]}")
            return False

    except subprocess.TimeoutExpired:
        print("[FAIL] CLI direct mode timed out")
        return False
    except Exception as e:
        print(f"[FAIL] CLI direct mode failed: {e}")
        return False
