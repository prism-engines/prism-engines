-- 2025_12_22_engine_validation_artifacts.sql
-- Engine Output Validation Schema: Phase-1 annotations for engine output quality control
--
-- Tables:
--   phase1.engine_validation - Validation results per (run_id, indicator_id, window_years, engine)
--   phase1.validation_reason_codes - Reason code definitions
-- Views:
--   phase1.v_validated_outputs - Only valid/degraded outputs
--   phase1.v_validation_summary - Aggregated validation summary


-- Validation results per (run_id, indicator_id, window_years, engine)
CREATE TABLE phase1.engine_validation (
...
CREATE TABLE phase1.validation_reason_codes (
...
INSERT OR IGNORE INTO phase1.validation_reason_codes VALUES
...
CREATE OR REPLACE VIEW phase1.v_validated_outputs AS
SELECT
    ev.*,
    CASE
...
CREATE OR REPLACE VIEW phase1.v_validation_summary AS
SELECT
    run_id,
    indicator_id,
    window_years,
