...
DUCKDB_PATH = Path(__file__).resolve().parents[1] / "duckdb" / "prism.duckdb"
...
class DuckDBPanelBuilder:
    ...
    def load_raw(self, path):
        """Load raw CSV or Parquet without ingesting into DB."""
        return self.con.execute(
            f"SELECT * FROM read_csv_auto('{path}')"
        ).df()

    def build_panel(self, folder, pattern="*.csv"):
        """Combine multiple CSVs into a unified panel."""
        folder = Path(folder)
