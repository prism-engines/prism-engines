...
def test_existing_code():
    ...
    optional_imports = [
        ...
        ("engine_core.lenses.volatility_lens", "VolatilityLens"),
    ]
    ...
    for module_path, class_name in critical_imports:
        try:
            ...
            critical_errors.append((module_path, str(e)))
    ...
    for module_path, class_name in optional_imports:
        try:
            ...
            print(f"⚠ {module_path}.{class_name} not available (optional)")
            warnings.append((module_path, str(e)))
    
    if critical_errors:
        print(f"\n❌ EXISTING CODE: {len(critical_errors)} critical imports failed")
        return False
    elif warnings:
        print(f"\n✅ EXISTING CODE: All critical imports passed ({len(warnings)} optional modules not available)")
        return True
    else:
