...
class GeometryEvolution:
    ...
    def summary(self) -> str:
        lines = [
            ...
            f"Time span: {self.windows[0].window_start.date()} to {self.windows[-1].window_end.date()}",
            "",
        ...
        for w in self.windows:
            ...
            lines.append(f"  {w.window_end.date()}: {bar} {w.n_clusters} clusters (ρ={w.mean_correlation:.2f})")
...
def get_available_indicators(conn: duckdb.DuckDBPyConnection) -> List[str]:
    ...
    FROM clean.indicator_values_v2
...
def get_date_range(conn: duckdb.DuckDBPyConnection, indicators: List[str]) -> Tuple[datetime, datetime]:
...
def analyze_rolling_windows(
    ...
    print(f"Analyzing {len(indicators)} indicators from {start_date.date()} to {end_date.date()}")
    ...
    while current_end <= end_date:
        ...
        print(f"  {current_end.date()}: {result.n_clusters} clusters, ρ={mean_corr:.3f}, conf={result.confidence:.1%}")
